...
def detect_bond_between_corners(im, corner1, corner2, bbox_width, angle_tol, hough_tol=10, window_spacing=15):
  ...
  n_true = 0
  for degree in np.linspace(0,1,round(np.linalg.norm(v)/window_spacing),endpoint=False):
    new_im = im.copy()
    ...
    for i,point in enumerate(point_list):
      ...
      cv2.line(display_im, tuple(map(int,  point1)), tuple(map(int, point2)), color=[255,0,0], thickness=2)
  ...
  for corner1 in corners:
    ...
    for idx in dist_sort[0:max_idx]:
      ...
      if line_detected:
        ...
        cv2.line(display_im, tuple(map(int, corner1)), tuple(map(int, corner2)), (0,0,255), 2)
  plt.imshow(display_im)
  plt.ion()
  plt.show()
